<?php
$pro = "simpan";
$nama_siswa = "";
$email = "";
$jurusan = "";
$telepon = "";
$username = "";
$password = "";
$level = "";
$status = "Aktif";
$keterangan = "";
?>

<link type="text/css" href="<?php echo "$PATH/base/"; ?>ui.all.css" rel="stylesheet" />
<script type="text/javascript" src="<?php echo "$PATH/"; ?>jquery-1.3.2.js"></script>
<script type="text/javascript" src="<?php echo "$PATH/"; ?>ui/ui.core.js"></script>
<script type="text/javascript" src="<?php echo "$PATH/"; ?>ui/ui.datepicker.js"></script>
<script type="text/javascript" src="<?php echo "$PATH/"; ?>ui/i18n/ui.datepicker-id.js"></script>

<link rel="stylesheet" href="assets/jsacordeon/jquery-ui.css">
<link rel="stylesheet" href="resources/demos/style.css">
<script src="assets/jsacordeon/jquery-1.12.4.js"></script>
<script src="assets/jsacordeon/jquery-ui.js"></script>
<script>
	$(function() {
		$("#accordion").accordion({
			collapsible: true
		});
	});
</script>

<?php
$sql = "select `id_siswa` from `$tbsiswa` order by `id_siswa` desc";
$jum = getJum($conn, $sql);
$kd = "PLJ";
if ($jum > 0) {
	$d = getField($conn, $sql);
	$idmax = $d['id_siswa'];
	$urut = substr($idmax, 3, 2) + 1; //01
	if ($urut < 10) {
		$idmax = "$kd" . "0" . $urut;
	} else {
		$idmax = "$kd" . $urut;
	}
} else {
	$idmax = "$kd" . "01";
}
$id_siswa = $idmax;
?>

<?php
if (isset($_GET["pro"]) && $_GET["pro"] == "ubah") {
	$id_siswa = $_GET["kode"];
	$sql = "select * from `$tbsiswa` where `id_siswa`='$id_siswa'";
	$d = getField($conn, $sql);
	$id_siswa = $d["id_siswa"];
	$id_siswa0 = $d["id_siswa"];
	$nama_siswa = $d["nama_siswa"];
	$email = $d["email"];
	$jurusan = $d["jurusan"];
	$telepon = $d["telepon"];
	$username = $d["username"];
	$password = $d["password"];
	$status = $d["status"];
	$keterangan = $d["keterangan"];
	$pro = "ubah";
}
?>


<div id="accordion">
	<h3>Input Data siswa</h3>
	<div>

		<div class="row">
            <div class="col-md-12">
              <div class="card">
                <div class="card-body">
				
                  <h4 class="card-title">Tambah Data siswa</h4><hr>
                    <form action="#" method="post" enctype="multipart/form-data">
					
					<div class="row">
						<div class="col-md-6">
						  <label for="id_siswa">ID siswa</label>
						  <input type="text" class="form-control" required name="id_siswa" id="id_siswa" value="<?php echo $id_siswa;?>" readonly>
						</div>
						<div class="col-md-6">
						  <label for="nama_siswa">Nama siswa</label>
						  <input type="text" class="form-control" required name="nama_siswa" id="nama_siswa" value="<?php echo $nama_siswa;?>" placeholder="">
						</div>
					</div>	
					
					<div class="row">
						<div class="col-md-6"><br>
						  <label for="email">Email </label>
						  <input type="email" class="form-control" required name="email" id="email" value="<?php echo $email;?>" placeholder="">
						</div>
						<div class="col-md-6"><br>
						  <label for="jurusan">Jurusan</label><br>
						  <input type="radio" name="jurusan" id="jurusan"  checked="checked" value="Teknik Komputer Jaringan" <?php if($jurusan=="Teknik Komputer Jaringan"){echo"checked";}?>/>Teknik Komputer Jaringan <br>
						  <input type="radio" name="jurusan" id="jurusan" value="Teknologi Laboratorium Medik" <?php if($jurusan=="Teknologi Laboratorium Medik"){echo"checked";}?>/>Teknologi Laboratorium Medik
						</div>
					</div>	
						
                    <div class="row">
						<div class="col-md-6"><br>
						  <label for="telepon">Telepon</label>
						  <input type="number" class="form-control" required name="telepon" id="telepon" value="<?php echo $telepon;?>" placeholder="">
						</div>
						<div class="col-md-6"><br>
						  <label for="username">Username</label>
						  <input type="text" class="form-control" required name="username" id="username" value="<?php echo $username;?>" placeholder="">
						</div>
					</div>
					
					<div class="row">
						<div class="col-md-6"><br>
						  <label for="password">Password</label>
						  <input type="password" class="form-control" required  name="password"  id="password" value="<?php echo $password;?>" placeholder="" />
						</div>
						<div class="col-md-6"><br>
						  <label for="status">Status</label><br>
						  <input type="radio" name="status" id="status"  checked="checked" value="Aktif" <?php if($status=="Aktif"){echo"checked";}?>/>Aktif <br>
						  <input type="radio" name="status" id="status" value="Tidak Aktif" <?php if($status=="Tidak Aktif"){echo"checked";}?>/>Tidak Aktif
						</div>
					</div>
					
					<div class="row">
						<div class="col-md-12"><br>
						  <label for="keterangan">Keterangan</label>
						  <textarea name="keterangan" class="form-control" placeholder="keterangan"><?php echo $keterangan;?></textarea>
						</div>
					</div>
					
                    <div class="row text-center">
						<div class="col-md-12"><br>
                            <button type="submit" name="Simpan" class="btn btn-success btn-sm ">Simpan</button>
							<input name="pro" type="hidden" id="pro" value="<?php echo $pro;?>" />
							<input name="id_siswa" type="hidden" id="id_siswa" value="<?php echo $id_siswa;?>" />
							<input name="id_siswa0" type="hidden" id="id_siswa0" value="<?php echo $id_siswa0;?>" />
							<a href="?mnu=siswa"><button class="btn btn-sm btn-danger">Batal</button></a>
						</div>
					</div>
                  </form>
				  
                </div>
              </div>
            </div>
		</div>
		
		
		
	</div>

	<?php
	$sqlc = "select distinct(`status`) from `$tbsiswa` order by `status` asc";
	$jumc = getJum($conn, $sqlc);
	if ($jumc < 1) {
		echo "<h1>Maaf data siswa belum tersedia</h1>";
	}
	$arrc = getData($conn, $sqlc);
	foreach ($arrc as $dc) {
		$status = $dc["status"];
	?>
	<br>
		<h3>Data siswa<?php echo $status ?>:</h3>
		<div>
		
<div class="pd-20 card-box mb-30">
					<div class="clearfix mb-20">
						<div class="pull-left">
							<h4 class="text-blue h4">Data siswa</h4>
							</div>
						</div>
					<div class="table-responsive">
						<table class="table table-striped">
						<thead>
							<tr>
								<th scope="col">No</th>
								<th scope="col">Id Siswa</th>
								<th scope="col">Nama Siswa</th>
								<th scope="col">Email</th>
								<th scope="col">Jurusan</th>
								<th scope="col">Telepon</th>
								<th scope="col">Username</th>
								<th scope="col">Keterangan</th>
								<th scope="col">Menu</th>
							</tr>
							<?php
				$sql = "select * from `$tbsiswa` where  `status`='$status' order by `id_siswa` asc";
				$jum = getJum($conn, $sql);
				if ($jum > 0) {
					$no=1;
							$arr = getData($conn, $sql);
					foreach ($arr as $d) {
						$id_siswa = $d["id_siswa"];
						$nama_siswa = ucwords($d["nama_siswa"]);
						$email = $d["email"];
						$jurusan = $d["jurusan"];
						$telepon = $d["telepon"];
						$username = $d["username"];
						$password = $d["password"];
						$status = $d["status"];
						$keterangan = $d["keterangan"];
						
						
						
					echo"</thead>
						<tbody>
							<tr>
							<td>$no</td>
							<td>$id_siswa</td>
							<td>$nama_siswa</td>
							<td>$email</td>
							<td>$jurusan</td>
							<td>$telepon</td>	
							<td>$username</td>
							<td>$keterangan</td>
							<td><div align='center'>
<a href='?mnu=siswa&pro=ubah&kode=$id_siswa'><img src='ypathicon/ub.png' title='ubah'></a>
<a href='?mnu=siswa&pro=hapus&kode=$id_siswa'><img src='ypathicon/ha.png' title='hapus' 
onClick='return confirm(\"Apakah Anda benar-benar akan menghapus \"$nama_siswa\" pada data siswa ?..\")'></a></div></td>
				</tr>";
						$no++;
					} //for dalam
				} //if
				else {
					echo "<tr><td colspan='6'><blink>Maaf, Data siswa belum tersedia...</blink></td></tr>";
				}
				?>
							
						</tbody>
					</table>
			
		</div>
	<?php } ?>
		</div>

		<?php
		if (isset($_POST["Simpan"])) {
			$pro = strip_tags($_POST["pro"]);
			$id_siswa = strip_tags($_POST["id_siswa"]);
			$id_siswa0 = strip_tags($_POST["id_siswa0"]);
			$nama_siswa = strip_tags($_POST["nama_siswa"]);
			$email = strip_tags($_POST["email"]);
			$jurusan = strip_tags($_POST["jurusan"]);
			$telepon = strip_tags($_POST["telepon"]);
			$username = strip_tags($_POST["username"]);
			$password = strip_tags($_POST["password"]);
			$status = strip_tags($_POST["status"]);
			$keterangan = strip_tags($_POST["keterangan"]);

			if ($pro == "simpan") {
				$sql = " INSERT INTO `$tbsiswa` (
					`id_siswa` ,
					`nama_siswa` ,
					`email` ,
					`jurusan` ,
					`telepon` ,
					`username` ,
					`password` ,
					`status` ,
					`keterangan`
					) VALUES (
					'$id_siswa', 
					'$nama_siswa',
					'$email',
					'$jurusan', 
					'$telepon',
					'$username',
					'$password',
					'$status',
					'$keterangan'
					)";

				$simpan = process($conn, $sql);
				if ($simpan) {
					echo "<script>alert('Data $nama_siswa berhasil disimpan !');document.location.href='?mnu=siswa';</script>";
				} else {
					echo "<script>alert('Data $nama_siswa gagal disimpan...');document.location.href='?mnu=siswa';</script>";
				}
			} else {
				$sql = "update `$tbsiswa` set 
					`nama_siswa`='$nama_siswa',
					`email`='$email',
					`jurusan`='$jurusan',
					`telepon`='$telepon' ,
					`username`='$username',
					`password`='$password',
					`status`='$status',
					`keterangan`='$keterangan'
					 where `id_siswa`='$id_siswa0'";
					 
				$ubah = process($conn, $sql);
				if ($ubah) {
					echo "<script>alert('Data $nama_siswa berhasil diubah !');document.location.href='?mnu=siswa';</script>";
				} else {
					echo "<script>alert('Data $nama_siswa gagal diubah...');document.location.href='?mnu=siswa';</script>";
				}
			} //else simpan
		}
		?>

		<?php
		if (isset($_GET["pro"]) && $_GET["pro"] == "hapus") {
			$id_siswa = $_GET["kode"];
			$sql = "delete from `$tbsiswa` where `id_siswa`='$id_siswa'";
			$hapus = process($conn, $sql);
			if ($hapus) {
				echo "<script>alert('Data $id_siswa berhasil dihapus !');document.location.href='?mnu=siswa';</script>";
			} else {
				echo "<script>alert('Data $id_siswa gagal dihapus...');document.location.href='?mnu=siswa';</script>";
			}
		}
		?>